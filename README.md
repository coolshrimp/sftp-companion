...
- **Sync Center** — scan both sides recursively in paired, row-aligned **Local** and **Server** file trees, color-coded by status (in sync, local newer, server newer, local only, server only). Search or filter by *Not in sync / Different / Local only / Server only / In sync*, then use the always-visible per-row direction control (download / skip / upload) or a one-click **Sync Shown** to let the newer side win everywhere. Diff, persistent ignore, and timestamp-only mark-in-sync are one click away too, alongside checkboxes, folder rollups, context menus, and bulk actions.
